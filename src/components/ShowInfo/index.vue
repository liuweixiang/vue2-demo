<template>
  <transition name="msgbox-fade">
    <div v-show="visible" :class="b('wrapper')" @click.self="handleWrapperClick">
      <div :class="[b(), customClass]">
        ...
        <div :class="b('content')">
          <div :class="b('tip', { desc: showDescription, 'only-alert': !hasContent })">
            <img v-if="showImg" :src="iconImg" />
            <slot>
              <p v-if="!dangerouslyUseHTMLString">{{ message }}</p>
              <p v-else v-html="message" />
            </slot>
          </div>
          <div v-if="hasContent" :class="b('notice-wrapper')">
            <p v-if="showNotice">{{ notice }}：</p>
            <ul v-if="showNotice" :class="b('notice')">
              <li v-for="(item, idx) in content" :key="idx">
                <div :class="b('notice-content', { 'has-more': item.content })">
                  ...
                  <MoreInfo v-if="visible" :text="item.content || ''" />
                </div>
              </li>
            </ul>
            <ul v-if="!showNotice" :class="b('notice')">
    ...
    watch: {
      visible(val) {
        if (val) {
          this.uid++
          if (
            [MessageTypeEnum.Alert, MessageTypeEnum.Confirm, MessageTypeEnum.Continue].includes(
              this.$type
            )
          ) {
            this.$nextTick(() => {
    ...
    methods: {
      ...
      handleWrapperClick() {
        this.closeOnClickModal &&
          this.handleAction(
            this.distinguishCancelAndClose ? ActionTypeEnum.Close : ActionTypeEnum.Cancel
          )
      },
      // 处理按钮点击事件
      handleAction(action) {
